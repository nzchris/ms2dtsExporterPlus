   ...
   void Mesh::read (InputStream &stream)
   {
      ...
      if (type == T_Skin) {
         ...
         // Inverse world transforms for bones used in vertex weighting
			int numNodeIndices;
			stream >> numNodeIndices;
			nodeIndex.resize(numNodeIndices);
         ...
         int numVIndices;
			stream >> numVIndices;
			vindex.resize(numVIndices);
			vbone.resize(numVIndices);
			vweight.resize(numVIndices);
         stream >> vindex;
         stream >> vbone;
         stream >> vweight;
...
/* TODO
      if (type == T_Sorted) {
         ...
         stream << (int) firstTVerts.size();
	      ...
	      int temp;
			stream >> temp; // alwaysWriteDepth deprecated
